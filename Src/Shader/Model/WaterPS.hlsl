...
cbuffer cbParam : register(b4)
{
	...
	float2 g_uv_scroll_speed; // (x方向, y方向)のスクロール速度
	float g_time;             // 時間（秒単位）
	float dummy2;
}
...
float4 main(PS_INPUT PSInput) : SV_TARGET0
{
	...
	float2 scrolledUV = frac(PSInput.uv + g_uv_scroll_speed * g_time);
	// テクスチャーの色を取得
	color = diffuseMapTexture.Sample(diffuseMapSampler, scrolledUV);
	//if (color.a < 0.01f)
	//{
	//	discard;
	//}
