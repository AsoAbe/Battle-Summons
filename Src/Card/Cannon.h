...
public:
	...
	// 砲台の基本サイズ
	static constexpr float DEFAULT_SIZE = 0.8f;

	// 回転速度
	static constexpr float ROTATE_SPEED_DEGREE = 2.0f;

	// 1フレーム分の時間
	static constexpr float FRAME_TIME = 1.0f / DEFAULT_FPS;
	...
	// 砲身の初期回転
	static constexpr float BARREL_ROT_X = -0.5f;
	static constexpr float BARREL_ROT_Y = 0.0f;
	static constexpr float BARREL_ROT_Z = 0.0f;

	// 砲身のX回転制限(最大)
	static constexpr float MAX_ANGLE_X
		= 15.0f * DEGREE_TO_RADIAN;
	
	// 砲身のX回転制限(最小)
	static constexpr float MIN_ANGLE_X
		= -30.0f * DEGREE_TO_RADIAN;
